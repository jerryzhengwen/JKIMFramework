...
typedef NS_ENUM(NSUInteger, JK_YYWebImageFromType) {
    ...
    JK_YYWebImageFromMemoryCacheFast,
    
    /// Fetched from memory cache.
    JK_YYWebImageFromMemoryCache,
    
    /// Fetched from disk cache.
    JK_YYWebImageFromDiskCache,
    
    /// Fetched from remote (web or file path).
    JK_YYWebImageFromRemote,
};

/// Indicated image fetch complete stage.
typedef NS_ENUM(NSInteger, JK_YYWebImageStage) {
    
    /// Incomplete, progressive image.
    JK_YYWebImageStageProgress  = -1,
    
    /// Cancelled.
    JK_YYWebImageStageCancelled = 0,
    
    /// Finished (succeed or failed).
    JK_YYWebImageStageFinished  = 1,
};
 ...
 */
typedef void(^JK_YYWebImageProgressBlock)(NSInteger receivedSize, NSInteger expectedSize);
 ...
 */
typedef UIImage * _Nullable (^JK_YYWebImageTransformBlock)(UIImage *image, NSURL *url);
 ...
 */
typedef void (^JK_YYWebImageCompletionBlock)(UIImage * _Nullable image,
                                                NSURL *url,
                                                JK_YYWebImageFromType from,
                                                JK_YYWebImageStage stage,
 ...
 */
@interface JK_YYWebImageManager : NSObject
 ...
 */
- (instancetype)initWithCache:(nullable JK_YYImageCache *)cache
 ...
 */
- (nullable JK_YYWebImageOperation *)requestImageWithURL:(NSURL *)url
 ...
 */
@property (nullable, nonatomic, strong) JK_YYImageCache *cache;
 ...
 */
@property (nullable, nonatomic, copy) JK_YYWebImageTransformBlock sharedTransformBlock;
