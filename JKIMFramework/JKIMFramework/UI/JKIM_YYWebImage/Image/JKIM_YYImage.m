...
@implementation JKIM_YYImage {
    JKIM_YYImageDecoder *_decoder;
    NSArray *_preloadedFrames;
    dispatch_semaphore_t _preloadedLock;
    NSUInteger _bytesPerFrame;
}

+ (JKIM_YYImage *)imageNamed:(NSString *)name {
...
+ (JKIM_YYImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (JKIM_YYImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (JKIM_YYImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        ...
        JKIM_YYImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    NSNumber *scale = [aDecoder decodeObjectForKey:@"JKIM_YYImageScale"];
    NSData *data = [aDecoder decodeObjectForKey:@"JKIM_YYImageData"];
    if (data.length) {
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    if (_decoder.data.length) {
        [aCoder encodeObject:@(self.scale) forKey:@"JKIM_YYImageScale"];
        [aCoder encodeObject:_decoder.data forKey:@"JKIM_YYImageData"];
    } else {
...
#pragma mark - protocol JKIM_YYAnimatedImage
