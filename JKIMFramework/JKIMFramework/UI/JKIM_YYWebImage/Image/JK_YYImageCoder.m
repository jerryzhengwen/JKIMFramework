...
CGColorSpaceRef JK_YYCGColorSpaceGetDeviceRGB() {
...
CGColorSpaceRef JK_YYCGColorSpaceGetDeviceGray() {
    static CGColorSpaceRef space;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
BOOL JK_YYCGColorSpaceIsDeviceRGB(CGColorSpaceRef space) {
    return space && CFEqual(space, JK_YYCGColorSpaceGetDeviceRGB());
}

BOOL JK_YYCGColorSpaceIsDeviceGray(CGColorSpaceRef space) {
    return space && CFEqual(space, JK_YYCGColorSpaceGetDeviceGray());
}
...
static BOOL YYCGImageDecodeToBitmapBufferWith32BitFormat(CGImageRef srcImage, vImage_Buffer *dest, CGBitmapInfo bitmapInfo) {
    ...
    destFormat.colorSpace = JK_YYCGColorSpaceGetDeviceRGB();
    ...
    CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, JK_YYCGColorSpaceGetDeviceRGB(), contextBitmapInfo);
...
CGImageRef JK_YYCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay) {
    ...
    if (decodeForDisplay) { //decode with redraw (may lose some precision)
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, JK_YYCGColorSpaceGetDeviceRGB(), bitmapInfo);
...
CGImageRef JK_YYCGImageCreateAffineTransformCopy(CGImageRef imageRef, CGAffineTransform transform, CGSize destSize, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    tmpImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, JK_YYCGColorSpaceGetDeviceRGB(), kCGImageAlphaFirst | kCGBitmapByteOrderDefault, tmpProvider, NULL, false, kCGRenderingIntentDefault);
    ...
    destImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, JK_YYCGColorSpaceGetDeviceRGB(), destBitmapInfo, destProvider, NULL, false, kCGRenderingIntentDefault);
...
UIImageOrientation JK_YYUIImageOrientationFromEXIFValue(NSInteger value) {
    switch (value) {
...
NSInteger JK_YYUIImageOrientationToEXIFValue(UIImageOrientation orientation) {
    switch (orientation) {
...
CGImageRef JK_YYCGImageCreateCopyWithOrientation(CGImageRef imageRef, UIImageOrientation orientation, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    return JK_YYCGImageCreateAffineTransformCopy(imageRef, transform, destSize, destBitmapInfo);
}

JK_YYImageType JK_YYImageDetectType(CFDataRef data) {
    if (!data) return JK_YYImageTypeUnknown;
    uint64_t length = CFDataGetLength(data);
    if (length < 16) return JK_YYImageTypeUnknown;
    ...
    switch (magic4) {
        ...
        case YY_FOUR_CC('i', 'c', 'n', 's'): { // ICNS
            return JK_YYImageTypeICNS;
        } break;
            
        case YY_FOUR_CC('G', 'I', 'F', '8'): { // GIF
            return JK_YYImageTypeGIF;
        } break;
            
        case YY_FOUR_CC(0x89, 'P', 'N', 'G'): {  // PNG
            uint32_t tmp = *((uint32_t *)(bytes + 4));
            if (tmp == YY_FOUR_CC('\r', '\n', 0x1A, '\n')) {
                return JK_YYImageTypePNG;
            }
        } break;
    ...
    switch (magic2) {
        ...
        case YY_TWO_CC('C', 'P'): { // BMP
            return JK_YYImageTypeBMP;
        }
        case YY_TWO_CC(0xFF, 0x4F): { // JPEG2000
            return JK_YYImageTypeJPEG2000;
        }
    }
    
    // JPG             FF D8 FF
    if (memcmp(bytes,"\377\330\377",3) == 0) return JK_YYImageTypeJPEG;
    
    // JP2
    if (memcmp(bytes + 4, "\152\120\040\040\015", 5) == 0) return JK_YYImageTypeJPEG2000;
    
    return JK_YYImageTypeUnknown;
}

CFStringRef JK_YYImageTypeToUTType(JK_YYImageType type) {
    switch (type) {
        case JK_YYImageTypeJPEG: return kUTTypeJPEG;
        case JK_YYImageTypeJPEG2000: return kUTTypeJPEG2000;
        case JK_YYImageTypeTIFF: return kUTTypeTIFF;
        case JK_YYImageTypeBMP: return kUTTypeBMP;
        case JK_YYImageTypeICO: return kUTTypeICO;
        case JK_YYImageTypeICNS: return kUTTypeAppleICNS;
        case JK_YYImageTypeGIF: return kUTTypeGIF;
        case JK_YYImageTypePNG: return kUTTypePNG;
        default: return NULL;
    }
}

JK_YYImageType JK_YYImageTypeFromUTType(CFStringRef uti) {
    static NSDictionary *dic;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        dic = @{(id)kUTTypeJPEG : @(JK_YYImageTypeJPEG),
                (id)kUTTypeJPEG2000 : @(JK_YYImageTypeJPEG2000),
                (id)kUTTypeTIFF : @(JK_YYImageTypeTIFF),
                (id)kUTTypeBMP : @(JK_YYImageTypeBMP),
                (id)kUTTypeICO : @(JK_YYImageTypeICO),
                (id)kUTTypeAppleICNS : @(JK_YYImageTypeICNS),
                (id)kUTTypeGIF : @(JK_YYImageTypeGIF),
                (id)kUTTypePNG : @(JK_YYImageTypePNG)};
    });
    if (!uti) return JK_YYImageTypeUnknown;
    NSNumber *num = dic[(__bridge __strong id)(uti)];
    return num.unsignedIntegerValue;
}

NSString *JK_YYImageTypeGetExtension(JK_YYImageType type) {
    switch (type) {
        case JK_YYImageTypeJPEG: return @"jpg";
        case JK_YYImageTypeJPEG2000: return @"jp2";
        case JK_YYImageTypeTIFF: return @"tiff";
        case JK_YYImageTypeBMP: return @"bmp";
        case JK_YYImageTypeICO: return @"ico";
        case JK_YYImageTypeICNS: return @"icns";
        case JK_YYImageTypeGIF: return @"gif";
        case JK_YYImageTypePNG: return @"png";
        case JK_YYImageTypeWebP: return @"webp";
        default: return nil;
    }
}

CFDataRef JK_YYCGImageCreateEncodedData(CGImageRef imageRef, JK_YYImageType type, CGFloat quality) {
    if (!imageRef) return nil;
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    
    if (type == JK_YYImageTypeWebP) {
#if YYIMAGE_WEBP_ENABLED
        if (quality == 1) {
            return JK_YYCGImageCreateEncodedWebPData(imageRef, YES, quality, 4, JK_YYImagePresetDefault);
        } else {
            return JK_YYCGImageCreateEncodedWebPData(imageRef, NO, quality, 4, JK_YYImagePresetDefault);
        }
#else
        return NULL;
#endif
    ...
    CFStringRef uti = JK_YYImageTypeToUTType(type);
    if (!uti) return nil;
...
BOOL JK_YYImageWebPAvailable() {
    return YES;
}

CFDataRef JK_YYCGImageCreateEncodedWebPData(CGImageRef imageRef, BOOL lossless, CGFloat quality, int compressLevel, JK_YYImagePreset preset) {
    if (!imageRef) return nil;
    size_t width = CGImageGetWidth(imageRef);
    size_t height = CGImageGetHeight(imageRef);
    ...
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    preset = preset > JK_YYImagePresetText ? JK_YYImagePresetDefault : preset;
...
NSUInteger JK_YYImageGetWebPFrameCount(CFDataRef webpData) {
...
CGImageRef JK_YYCGImageCreateWithWebPData(CFDataRef webpData,
    ...
    imageRef = CGImageCreate(canvasWidth, canvasHeight, bitsPerComponent, bitsPerPixel, bytesPerRow, JK_YYCGColorSpaceGetDeviceRGB(), bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
...
@implementation JK_YYImageFrame
+ (instancetype)frameWithImage:(UIImage *)image {
    JK_YYImageFrame *frame = [self new];
    frame.image = image;
    return frame;
}
- (id)copyWithZone:(NSZone *)zone {
    JK_YYImageFrame *frame = [self.class new];
    frame.index = _index;
    frame.width = _width;
...
@interface JK_YYImageDecoderFrame : JK_YYImageFrame
...
@implementation JK_YYImageDecoderFrame
- (id)copyWithZone:(NSZone *)zone {
    JK_YYImageDecoderFrame *frame = [super copyWithZone:zone];
...
@implementation JK_YYImageDecoder {
...
+ (instancetype)decoderWithData:(NSData *)data scale:(CGFloat)scale {
    if (!data) return nil;
    JK_YYImageDecoder *decoder = [[JK_YYImageDecoder alloc] initWithScale:scale];
...
- (JK_YYImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    JK_YYImageFrame *result = nil;
    pthread_mutex_lock(&_lock);
...
- (NSTimeInterval)frameDurationAtIndex:(NSUInteger)index {
    ...
    if (index < _frames.count) {
        result = ((JK_YYImageDecoderFrame *)_frames[index]).duration;
    }
...
- (BOOL)_updateData:(NSData *)data final:(BOOL)final {
    ...
    JK_YYImageType type = JK_YYImageDetectType((__bridge CFDataRef)data);
...
- (JK_YYImageFrame *)_frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    if (index >= _frames.count) return 0;
    JK_YYImageDecoderFrame *frame = [(JK_YYImageDecoderFrame *)_frames[index] copy];
    BOOL decoded = NO;
    BOOL extendToCanvas = NO;
    if (_type != JK_YYImageTypeICO && decodeForDisplay) { // ICO contains multi-size frame and should not extend to canvas.
        extendToCanvas = YES;
    }
    ...
    if (!_needBlend) {
        ...
        if (decodeForDisplay && !decoded) {
            CGImageRef imageRefDecoded = JK_YYCGImageCreateDecodedCopy(imageRef, YES);
            if (imageRefDecoded) {
                CFRelease(imageRef);
    ...
    if (_blendFrameIndex + 1 == frame.index) {
        ...
        if (frame.blendFromIndex == frame.index) {
            ...
            if (frame.dispose == JK_YYImageDisposeBackground) {
    ...
    if (extendToCanvas) {
        ...
        frame.offsetX = 0;
        frame.offsetY = 0;
        frame.dispose = JK_YYImageDisposeNone;
        frame.blend = JK_YYImageBlendNone;
    }
    return frame;
}
...
- (void)_updateSource {
    switch (_type) {
        case JK_YYImageTypeWebP: {
            [self _updateSourceWebP];
        } break;
            
        case JK_YYImageTypePNG: {
...
- (void)_updateSourceWebP {
    ...
    if (WebPDemuxGetFrame(demuxer, 1, &iter)) { // one-based index...
        do {
            JK_YYImageDecoderFrame *frame = [JK_YYImageDecoderFrame new];
            [frames addObject:frame];
            if (iter.dispose_method == WEBP_MUX_DISPOSE_BACKGROUND) {
                frame.dispose = JK_YYImageDisposeBackground;
            }
            if (iter.blend_method == WEBP_MUX_BLEND) {
                frame.blend = JK_YYImageBlendOver;
            }
    ...
    for (uint32_t i = 0; i < apng->apng_frame_num; i++) {
        ...
        switch (fi->frame_control.dispose_op) {
            ...
            case YY_PNG_DISPOSE_OP_PREVIOUS: {
                frame.dispose = JK_YYImageDisposePrevious;
            } break;
            default: {
                frame.dispose = JK_YYImageDisposeNone;
            } break;
        ...
        if (frame.blend == JK_YYImageBlendNone && frame.isFullSize) {
            frame.blendFromIndex  = i;
            if (frame.dispose != JK_YYImageDisposePrevious) lastBlendIndex = i;
        } else {
            if (frame.dispose == JK_YYImageDisposeBackground && frame.isFullSize) {
                frame.blendFromIndex = lastBlendIndex;
                lastBlendIndex = i + 1;
            } else {
                frame.blendFromIndex = lastBlendIndex;
...
- (void)_updateSourceImageIO {
    ...
    if (!_finalized) { // ignore multi-frame before finalized
        ...
        if (_type == JK_YYImageTypePNG) { // use custom apng decoder and ignore multi-frame
            _frameCount = 1;
        }
        if (_type == JK_YYImageTypeGIF) { // get gif loop count
            CFDictionaryRef properties = CGImageSourceCopyProperties(_source, NULL);
            if (properties) {
        ...
        if (properties) {
            ...
            if (i == 0 && _width + _height == 0) { // init first frame
                ...
                if (value) {
                    ...
                    _orientation = JK_YYUIImageOrientationFromEXIFValue(orientationValue);
                }
            }
            CFRelease(properties);
        }
...
- (CGImageRef)_newUnblendedImageAtIndex:(NSUInteger)index
    ...
    JK_YYImageDecoderFrame *frame = _frames[index];
    ...
    if (_source) {
        ...
        if (imageRef && extendToCanvas) {
            ...
            if (width == _width && height == _height) {
                CGImageRef imageRefExtended = JK_YYCGImageCreateDecodedCopy(imageRef, YES);
                if (imageRefExtended) {
                    CFRelease(imageRef);
                ...
                CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, JK_YYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
                if (context) {
    ...
    if (_apngSource) {
        ...
        if (!imageRef) return NULL;
        if (extendToCanvas) {
            CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, JK_YYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst); //bgrA
            if (context) {
    ...
    if (_webpSource) {
        ...
        CGImageRef image = CGImageCreate(width, height, bitsPerComponent, bitsPerPixel, bytesPerRow, JK_YYCGColorSpaceGetDeviceRGB(), bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
        CFRelease(provider);
        if (decoded) *decoded = YES;
        return image;
    }
...
- (BOOL)_createBlendContextIfNeeded {
    if (!_blendCanvas) {
        _blendFrameIndex = NSNotFound;
        _blendCanvas = CGBitmapContextCreate(NULL, _width, _height, 8, 0, JK_YYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    BOOL suc = _blendCanvas != NULL;
    return suc;
}

- (void)_blendImageWithFrame:(JK_YYImageDecoderFrame *)frame {
    if (frame.dispose == JK_YYImageDisposePrevious) {
        // nothing
    } else if (frame.dispose == JK_YYImageDisposeBackground) {
        CGContextClearRect(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height));
    } else { // no dispose
        if (frame.blend == JK_YYImageBlendOver) {
...
- (CGImageRef)_newBlendedImageWithFrame:(JK_YYImageDecoderFrame *)frame CF_RETURNS_RETAINED{
    CGImageRef imageRef = NULL;
    if (frame.dispose == JK_YYImageDisposePrevious) {
        if (frame.blend == JK_YYImageBlendOver) {
...
@implementation JK_YYImageEncoder {
...
- (instancetype)init {
    ...
    return [self initWithType:JK_YYImageTypeUnknown];
}

- (instancetype)initWithType:(JK_YYImageType)type {
    if (type == JK_YYImageTypeUnknown || type >= JK_YYImageTypeOther) {
    ...
    switch (type) {
        case JK_YYImageTypeJPEG:
        case JK_YYImageTypeJPEG2000: {
            _quality = 0.9;
        } break;
        case JK_YYImageTypeTIFF:
        case JK_YYImageTypeBMP:
        case JK_YYImageTypeGIF:
        case JK_YYImageTypeICO:
        case JK_YYImageTypeICNS:
        case JK_YYImageTypePNG: {
            _quality = 1;
            _lossless = YES;
        } break;
...
- (CGImageDestinationRef)_newImageDestination:(id)dest imageCount:(NSUInteger)count CF_RETURNS_RETAINED {
    ...
    if ([dest isKindOfClass:[NSString class]]) {
        ...
        if (url) {
            destination = CGImageDestinationCreateWithURL((CFURLRef)url, JK_YYImageTypeToUTType(_type), count, NULL);
        }
    } else if ([dest isKindOfClass:[NSMutableData class]]) {
        destination = CGImageDestinationCreateWithData((CFMutableDataRef)dest, JK_YYImageTypeToUTType(_type), count, NULL);
    }
    return destination;
}
...
- (void)_encodeImageWithDestination:(CGImageDestinationRef)destination imageCount:(NSUInteger)count {
    ...
    for (int i = 0; i < count; i++) {
        @autoreleasepool {
            ...
            if (_type == JK_YYImageTypeGIF && count > 1) {
            ...
            if ([imageSrc isKindOfClass:[UIImage class]]) {
                ...
                if (image.imageOrientation != UIImageOrientationUp && image.CGImage) {
                    ...
                    CGImageRef rotated = JK_YYCGImageCreateCopyWithOrientation(image.CGImage, image.imageOrientation, info);
                    if (rotated) {
...
- (CGImageRef)_newCGImageFromIndex:(NSUInteger)index decoded:(BOOL)decoded CF_RETURNS_RETAINED {
    ...
    if (image.imageOrientation != UIImageOrientationUp) {
        return JK_YYCGImageCreateCopyWithOrientation(imageRef, image.imageOrientation, kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    if (decoded) {
        return JK_YYCGImageCreateDecodedCopy(imageRef, YES);
    }
...
- (NSData *)_encodeWithImageIO {
    ...
    NSUInteger count = _type == JK_YYImageTypeGIF ? _images.count : 1;
...
- (NSData *)_encodeAPNG {
    ...
    for (int i = 0; i < _images.count; i++) {
        ...
        CFDataRef frameData = JK_YYCGImageCreateEncodedData(decoded, JK_YYImageTypePNG, 1);
        CFRelease(decoded);
    ...
    if (firstFrameSize.width < canvasWidth || firstFrameSize.height < canvasHeight) {
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, canvasWidth, canvasHeight, 8,
                                                     0, JK_YYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
        if (!context) {
            CFRelease(decoded);
            return nil;
        }
        ...
        CFDataRef frameData = JK_YYCGImageCreateEncodedData(extendedImage, JK_YYImageTypePNG, 1);
        if (!frameData) {
...
- (NSData *)_encodeWebP {
    ...
    for (NSUInteger i = 0; i < _images.count; i++) {
        ...
        if (!image) return nil;
        CFDataRef frameData = JK_YYCGImageCreateEncodedWebPData(image, _lossless, _quality, 4, JK_YYImagePresetDefault);
        CFRelease(image);
        if (!frameData) return nil;
...
- (NSData *)encode {
    ...
    if (_type == JK_YYImageTypePNG) return [self _encodeAPNG];
    if (_type == JK_YYImageTypeWebP) return [self _encodeWebP];
    return nil;
}
...
+ (NSData *)encodeImage:(UIImage *)image type:(JK_YYImageType)type quality:(CGFloat)quality {
    JK_YYImageEncoder *encoder = [[JK_YYImageEncoder alloc] initWithType:type];
    encoder.quality = quality;
    [encoder addImage:image duration:0];
    return [encoder encode];
}

+ (NSData *)encodeImageWithDecoder:(JK_YYImageDecoder *)decoder type:(JK_YYImageType)type quality:(CGFloat)quality {
...
@implementation UIImage (JK_YYImageCoder)

- (instancetype)yy_imageByDecoded {
    if (self.yy_isDecodedForDisplay) return self;
    CGImageRef imageRef = self.CGImage;
    if (!imageRef) return self;
    CGImageRef newImageRef = JK_YYCGImageCreateDecodedCopy(imageRef, YES);
...
- (BOOL)yy_isDecodedForDisplay {
    if (self.images.count > 1 || [self isKindOfClass:[JK_YYSpriteSheetImage class]]) return YES;
...
- (NSData *)_yy_dataRepresentationForSystem:(BOOL)forSystem {
    NSData *data = nil;
    if ([self isKindOfClass:[JK_YYImage class]]) {
        JK_YYImage *image = (id)self;
        if (image.animatedImageData) {
            if (forSystem) { // system only support GIF and PNG
                if (image.animatedImageType == JK_YYImageTypeGIF ||
                    image.animatedImageType == JK_YYImageTypePNG) {
    ...
    if (!data) {
        ...
        if (imageRef) {
            ...
            if (self.imageOrientation != UIImageOrientationUp) {
                CGImageRef rotated = JK_YYCGImageCreateCopyWithOrientation(imageRef, self.imageOrientation, bitmapInfo | alphaInfo);
                if (rotated) {
                    CFRelease(imageRef);
                    imageRef = rotated;
                }
