...
@implementation JK_YYImage {
    JK_YYImageDecoder *_decoder;
    NSArray *_preloadedFrames;
    dispatch_semaphore_t _preloadedLock;
    NSUInteger _bytesPerFrame;
}

+ (JK_YYImage *)imageNamed:(NSString *)name {
...
+ (JK_YYImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (JK_YYImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (JK_YYImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        ...
        JK_YYImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
