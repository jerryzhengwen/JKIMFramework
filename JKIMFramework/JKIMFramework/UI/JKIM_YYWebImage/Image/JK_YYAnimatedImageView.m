 ...
 */
@interface JK_YYImageWeakProxy : NSProxy
@property (nonatomic, weak, readonly) id target;
- (instancetype)initWithTarget:(id)target;
+ (instancetype)proxyWithTarget:(id)target;
@end

@implementation JK_YYImageWeakProxy
- (instancetype)initWithTarget:(id)target {
    _target = target;
    return self;
}
+ (instancetype)proxyWithTarget:(id)target {
    return [[JK_YYImageWeakProxy alloc] initWithTarget:target];
}
...
@interface JK_YYAnimatedImageView() {
    @package
...
@interface JK_YYAnimatedImageViewFetchOperation : NSOperation
@property (nonatomic, weak) JK_YYAnimatedImageView *view;
@property (nonatomic, assign) NSUInteger nextIndex;
@property (nonatomic, strong) UIImage <YYAnimatedImage> *curImage;
@end

@implementation JK_YYAnimatedImageViewFetchOperation
- (void)main {
    __strong JK_YYAnimatedImageView *view = _view;
    if (!view) return;
...
@implementation JK_YYAnimatedImageView
...
- (void)resetAnimated {
    dispatch_once(&_onceToken, ^{
        ...
        _link = [CADisplayLink displayLinkWithTarget:[JK_YYImageWeakProxy proxyWithTarget:self] selector:@selector(step:)];
        if (_runloopMode) {
...
- (void)step:(CADisplayLink *)link {
    ...
    if (!bufferIsFull && _requestQueue.operationCount == 0) { // if some work not finished, wait for next opportunity
        JK_YYAnimatedImageViewFetchOperation *operation = [JK_YYAnimatedImageViewFetchOperation new];
        operation.view = self;
