...
#import "webp/mux.h"
#else
#define JKIM_YYImage_WEBP_ENABLED 0
#endif
#endif
...
static inline size_t JKIM_YYImageByteAlign(size_t size, size_t alignment) {
    return ((size + (alignment - 1)) / alignment) * alignment;
}

/// Convert degree to radians
static inline CGFloat JKIM_YYImageDegreesToRadians(CGFloat degrees) {
    return degrees * M_PI / 180;
}

CGColorSpaceRef JKIM_YYCGColorSpaceGetDeviceRGB() {
...
CGColorSpaceRef JKIM_YYCGColorSpaceGetDeviceGray() {
    static CGColorSpaceRef space;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
BOOL JKIM_YYCGColorSpaceIsDeviceRGB(CGColorSpaceRef space) {
    return space && CFEqual(space, JKIM_YYCGColorSpaceGetDeviceRGB());
}

BOOL JKIM_YYCGColorSpaceIsDeviceGray(CGColorSpaceRef space) {
    return space && CFEqual(space, JKIM_YYCGColorSpaceGetDeviceGray());
}
...
static BOOL YYCGImageDecodeToBitmapBufferWith32BitFormat(CGImageRef srcImage, vImage_Buffer *dest, CGBitmapInfo bitmapInfo) {
    ...
    destFormat.colorSpace = JKIM_YYCGColorSpaceGetDeviceRGB();
    ...
    CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, JKIM_YYCGColorSpaceGetDeviceRGB(), contextBitmapInfo);
...
CGImageRef JKIM_YYCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay) {
    ...
    if (decodeForDisplay) { //decode with redraw (may lose some precision)
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, JKIM_YYCGColorSpaceGetDeviceRGB(), bitmapInfo);
...
CGImageRef JKIM_YYCGImageCreateAffineTransformCopy(CGImageRef imageRef, CGAffineTransform transform, CGSize destSize, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    size_t destBytesPerRow = JKIM_YYImageByteAlign(destWidth * 4, 32);
    ...
    tmpImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, JKIM_YYCGColorSpaceGetDeviceRGB(), kCGImageAlphaFirst | kCGBitmapByteOrderDefault, tmpProvider, NULL, false, kCGRenderingIntentDefault);
    ...
    destImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, JKIM_YYCGColorSpaceGetDeviceRGB(), destBitmapInfo, destProvider, NULL, false, kCGRenderingIntentDefault);
...
UIImageOrientation JKIM_YYUIImageOrientationFromEXIFValue(NSInteger value) {
    switch (value) {
...
NSInteger JKIM_YYUIImageOrientationToEXIFValue(UIImageOrientation orientation) {
    switch (orientation) {
...
CGImageRef JKIM_YYCGImageCreateCopyWithOrientation(CGImageRef imageRef, UIImageOrientation orientation, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    switch (orientation) {
        case UIImageOrientationDown: {
            transform = CGAffineTransformMakeRotation(JKIM_YYImageDegreesToRadians(180));
            transform = CGAffineTransformTranslate(transform, -(CGFloat)width, -(CGFloat)height);
        } break;
        case UIImageOrientationLeft: {
            transform = CGAffineTransformMakeRotation(JKIM_YYImageDegreesToRadians(90));
    ...
    return JKIM_YYCGImageCreateAffineTransformCopy(imageRef, transform, destSize, destBitmapInfo);
}

JKIM_YYImageType JKIM_YYImageDetectType(CFDataRef data) {
    if (!data) return JKIM_YYImageTypeUnknown;
    uint64_t length = CFDataGetLength(data);
    if (length < 16) return JKIM_YYImageTypeUnknown;
    ...
    switch (magic4) {
        ...
        case YY_FOUR_CC('i', 'c', 'n', 's'): { // ICNS
            return JKIM_YYImageTypeICNS;
        } break;
            
        case YY_FOUR_CC('G', 'I', 'F', '8'): { // GIF
            return JKIM_YYImageTypeGIF;
        } break;
            
        case YY_FOUR_CC(0x89, 'P', 'N', 'G'): {  // PNG
            uint32_t tmp = *((uint32_t *)(bytes + 4));
            if (tmp == YY_FOUR_CC('\r', '\n', 0x1A, '\n')) {
                return JKIM_YYImageTypePNG;
            }
        } break;
        ...
        case YY_FOUR_CC('R', 'I', 'F', 'F'): { // WebP
            ...
            if (tmp == YY_FOUR_CC('W', 'E', 'B', 'P')) {
                return JKIM_YYImageTypeWebP;
            }
        } break;
        /*
        case YY_FOUR_CC('B', 'P', 'G', 0xFB): { // BPG
            return JKIM_YYImageTypeBPG;
        } break;
        */
    }
    ...
    switch (magic2) {
        ...
        case YY_TWO_CC('C', 'P'): { // BMP
            return JKIM_YYImageTypeBMP;
        }
        case YY_TWO_CC(0xFF, 0x4F): { // JPEG2000
            return JKIM_YYImageTypeJPEG2000;
        }
    }
    
    // JPG             FF D8 FF
    if (memcmp(bytes,"\377\330\377",3) == 0) return JKIM_YYImageTypeJPEG;
    
    // JP2
    if (memcmp(bytes + 4, "\152\120\040\040\015", 5) == 0) return JKIM_YYImageTypeJPEG2000;
    
    return JKIM_YYImageTypeUnknown;
}

CFStringRef JKIM_YYImageTypeToUTType(JKIM_YYImageType type) {
    switch (type) {
        case JKIM_YYImageTypeJPEG: return kUTTypeJPEG;
        case JKIM_YYImageTypeJPEG2000: return kUTTypeJPEG2000;
        case JKIM_YYImageTypeTIFF: return kUTTypeTIFF;
        case JKIM_YYImageTypeBMP: return kUTTypeBMP;
        case JKIM_YYImageTypeICO: return kUTTypeICO;
        case JKIM_YYImageTypeICNS: return kUTTypeAppleICNS;
        case JKIM_YYImageTypeGIF: return kUTTypeGIF;
        case JKIM_YYImageTypePNG: return kUTTypePNG;
        default: return NULL;
    }
}

JKIM_YYImageType JKIM_YYImageTypeFromUTType(CFStringRef uti) {
    static NSDictionary *dic;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        dic = @{(id)kUTTypeJPEG : @(JKIM_YYImageTypeJPEG),
                (id)kUTTypeJPEG2000 : @(JKIM_YYImageTypeJPEG2000),
                (id)kUTTypeTIFF : @(JKIM_YYImageTypeTIFF),
                (id)kUTTypeBMP : @(JKIM_YYImageTypeBMP),
                (id)kUTTypeICO : @(JKIM_YYImageTypeICO),
                (id)kUTTypeAppleICNS : @(JKIM_YYImageTypeICNS),
                (id)kUTTypeGIF : @(JKIM_YYImageTypeGIF),
                (id)kUTTypePNG : @(JKIM_YYImageTypePNG)};
    });
    if (!uti) return JKIM_YYImageTypeUnknown;
    NSNumber *num = dic[(__bridge __strong id)(uti)];
    return num.unsignedIntegerValue;
}

NSString *JKIM_YYImageTypeGetExtension(JKIM_YYImageType type) {
    switch (type) {
        case JKIM_YYImageTypeJPEG: return @"jpg";
        case JKIM_YYImageTypeJPEG2000: return @"jp2";
        case JKIM_YYImageTypeTIFF: return @"tiff";
        case JKIM_YYImageTypeBMP: return @"bmp";
        case JKIM_YYImageTypeICO: return @"ico";
        case JKIM_YYImageTypeICNS: return @"icns";
        case JKIM_YYImageTypeGIF: return @"gif";
        case JKIM_YYImageTypePNG: return @"png";
        case JKIM_YYImageTypeWebP: return @"webp";
        default: return nil;
    }
}

CFDataRef JKIM_YYCGImageCreateEncodedData(CGImageRef imageRef, JKIM_YYImageType type, CGFloat quality) {
    if (!imageRef) return nil;
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    
    if (type == JKIM_YYImageTypeWebP) {
#if JKIM_YYImage_WEBP_ENABLED
        if (quality == 1) {
            return JKIM_YYCGImageCreateEncodedWebPData(imageRef, YES, quality, 4, JKIM_YYImagePresetDefault);
        } else {
            return JKIM_YYCGImageCreateEncodedWebPData(imageRef, NO, quality, 4, JKIM_YYImagePresetDefault);
        }
#else
        return NULL;
#endif
    ...
    CFStringRef uti = JKIM_YYImageTypeToUTType(type);
    if (!uti) return nil;
...
BOOL JKIM_YYImageWebPAvailable() {
    return YES;
}

CFDataRef JKIM_YYCGImageCreateEncodedWebPData(CGImageRef imageRef, BOOL lossless, CGFloat quality, int compressLevel, JKIM_YYImagePreset preset) {
    if (!imageRef) return nil;
    size_t width = CGImageGetWidth(imageRef);
    size_t height = CGImageGetHeight(imageRef);
    ...
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    preset = preset > JKIM_YYImagePresetText ? JKIM_YYImagePresetDefault : preset;
...
NSUInteger JKIM_YYImageGetWebPFrameCount(CFDataRef webpData) {
...
CGImageRef JKIM_YYCGImageCreateWithWebPData(CFDataRef webpData,
    ...
    bitsPerComponent = 8;
    bitsPerPixel = 32;
    bytesPerRow = JKIM_YYImageByteAlign(bitsPerPixel / 8 * canvasWidth, 32);
    ...
    imageRef = CGImageCreate(canvasWidth, canvasHeight, bitsPerComponent, bitsPerPixel, bytesPerRow, JKIM_YYCGColorSpaceGetDeviceRGB(), bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
...
@implementation JKIM_YYImageFrame
+ (instancetype)frameWithImage:(UIImage *)image {
    JKIM_YYImageFrame *frame = [self new];
    frame.image = image;
    return frame;
}
- (id)copyWithZone:(NSZone *)zone {
    JKIM_YYImageFrame *frame = [self.class new];
    frame.index = _index;
    frame.width = _width;
...
@interface _JKIM_YYImageDecoderFrame : JKIM_YYImageFrame
...
@implementation _JKIM_YYImageDecoderFrame
- (id)copyWithZone:(NSZone *)zone {
    _JKIM_YYImageDecoderFrame *frame = [super copyWithZone:zone];
...
@implementation JKIM_YYImageDecoder {
...
+ (instancetype)decoderWithData:(NSData *)data scale:(CGFloat)scale {
    if (!data) return nil;
    JKIM_YYImageDecoder *decoder = [[JKIM_YYImageDecoder alloc] initWithScale:scale];
...
- (JKIM_YYImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    JKIM_YYImageFrame *result = nil;
    pthread_mutex_lock(&_lock);
...
- (NSTimeInterval)frameDurationAtIndex:(NSUInteger)index {
    ...
    if (index < _frames.count) {
        result = ((_JKIM_YYImageDecoderFrame *)_frames[index]).duration;
    }
...
- (BOOL)_updateData:(NSData *)data final:(BOOL)final {
    ...
    JKIM_YYImageType type = JKIM_YYImageDetectType((__bridge CFDataRef)data);
...
- (JKIM_YYImageFrame *)_frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    if (index >= _frames.count) return 0;
    _JKIM_YYImageDecoderFrame *frame = [(_JKIM_YYImageDecoderFrame *)_frames[index] copy];
    BOOL decoded = NO;
    BOOL extendToCanvas = NO;
    if (_type != JKIM_YYImageTypeICO && decodeForDisplay) { // ICO contains multi-size frame and should not extend to canvas.
        extendToCanvas = YES;
    }
    ...
    if (!_needBlend) {
        ...
        if (decodeForDisplay && !decoded) {
            CGImageRef imageRefDecoded = JKIM_YYCGImageCreateDecodedCopy(imageRef, YES);
            if (imageRefDecoded) {
                CFRelease(imageRef);
    ...
    if (_blendFrameIndex + 1 == frame.index) {
        ...
        if (frame.blendFromIndex == frame.index) {
            ...
            if (frame.dispose == JKIM_YYImageDisposeBackground) {
    ...
    if (extendToCanvas) {
        ...
        frame.offsetX = 0;
        frame.offsetY = 0;
        frame.dispose = JKIM_YYImageDisposeNone;
        frame.blend = JKIM_YYImageBlendNone;
    }
    return frame;
}
...
- (void)_updateSource {
    switch (_type) {
        case JKIM_YYImageTypeWebP: {
            [self _updateSourceWebP];
        } break;
            
        case JKIM_YYImageTypePNG: {
...
- (void)_updateSourceWebP {
    ...
    if (WebPDemuxGetFrame(demuxer, 1, &iter)) { // one-based index...
        do {
            _JKIM_YYImageDecoderFrame *frame = [_JKIM_YYImageDecoderFrame new];
            [frames addObject:frame];
            if (iter.dispose_method == WEBP_MUX_DISPOSE_BACKGROUND) {
                frame.dispose = JKIM_YYImageDisposeBackground;
            }
            if (iter.blend_method == WEBP_MUX_BLEND) {
                frame.blend = JKIM_YYImageBlendOver;
            }
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSLog(@"[%s: %d] WebP is not available, check the documentation to see how to install WebP component: https://github.com/ibireme/JKIM_YYImage#installation", func, line);
    });
#endif
}
    ...
    for (uint32_t i = 0; i < apng->apng_frame_num; i++) {
        ...
        switch (fi->frame_control.dispose_op) {
            ...
            case YY_PNG_DISPOSE_OP_PREVIOUS: {
                frame.dispose = JKIM_YYImageDisposePrevious;
            } break;
            default: {
                frame.dispose = JKIM_YYImageDisposeNone;
            } break;
        ...
        if (frame.blend == JKIM_YYImageBlendNone && frame.isFullSize) {
            frame.blendFromIndex  = i;
            if (frame.dispose != JKIM_YYImageDisposePrevious) lastBlendIndex = i;
        } else {
            if (frame.dispose == JKIM_YYImageDisposeBackground && frame.isFullSize) {
                frame.blendFromIndex = lastBlendIndex;
                lastBlendIndex = i + 1;
            } else {
                frame.blendFromIndex = lastBlendIndex;
...
- (void)_updateSourceImageIO {
    ...
    if (!_finalized) { // ignore multi-frame before finalized
        ...
        if (_type == JKIM_YYImageTypePNG) { // use custom apng decoder and ignore multi-frame
            _frameCount = 1;
        }
        if (_type == JKIM_YYImageTypeGIF) { // get gif loop count
            CFDictionaryRef properties = CGImageSourceCopyProperties(_source, NULL);
            if (properties) {
        ...
        if (properties) {
            ...
            if (i == 0 && _width + _height == 0) { // init first frame
                ...
                if (value) {
                    ...
                    _orientation = JKIM_YYUIImageOrientationFromEXIFValue(orientationValue);
                }
            }
            CFRelease(properties);
        }
...
- (CGImageRef)_newUnblendedImageAtIndex:(NSUInteger)index
    ...
    _JKIM_YYImageDecoderFrame *frame = _frames[index];
    ...
    if (_source) {
        ...
        if (imageRef && extendToCanvas) {
            ...
            if (width == _width && height == _height) {
                CGImageRef imageRefExtended = JKIM_YYCGImageCreateDecodedCopy(imageRef, YES);
                if (imageRefExtended) {
                    CFRelease(imageRef);
                ...
                CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, JKIM_YYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
                if (context) {
    ...
    if (_apngSource) {
        ...
        if (!imageRef) return NULL;
        if (extendToCanvas) {
            CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, JKIM_YYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst); //bgrA
            if (context) {
    ...
    if (_webpSource) {
        ...
        size_t bytesPerRow = JKIM_YYImageByteAlign(bitsPerPixel / 8 * width, 32);
        ...
        CGImageRef image = CGImageCreate(width, height, bitsPerComponent, bitsPerPixel, bytesPerRow, JKIM_YYCGColorSpaceGetDeviceRGB(), bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
        CFRelease(provider);
        if (decoded) *decoded = YES;
        return image;
    }
...
- (BOOL)_createBlendContextIfNeeded {
    if (!_blendCanvas) {
        _blendFrameIndex = NSNotFound;
        _blendCanvas = CGBitmapContextCreate(NULL, _width, _height, 8, 0, JKIM_YYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    BOOL suc = _blendCanvas != NULL;
    return suc;
}

- (void)_blendImageWithFrame:(_JKIM_YYImageDecoderFrame *)frame {
    if (frame.dispose == JKIM_YYImageDisposePrevious) {
        // nothing
    } else if (frame.dispose == JKIM_YYImageDisposeBackground) {
        CGContextClearRect(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height));
    } else { // no dispose
        if (frame.blend == JKIM_YYImageBlendOver) {
...
- (CGImageRef)_newBlendedImageWithFrame:(_JKIM_YYImageDecoderFrame *)frame CF_RETURNS_RETAINED{
    CGImageRef imageRef = NULL;
    if (frame.dispose == JKIM_YYImageDisposePrevious) {
        if (frame.blend == JKIM_YYImageBlendOver) {
...
@implementation JKIM_YYImageEncoder {
    NSMutableArray *_images;
    NSMutableArray *_durations;
}

- (instancetype)init {
    @throw [NSException exceptionWithName:@"JKIM_YYImageEncoder init error" reason:@"JKIM_YYImageEncoder must be initialized with a type. Use 'initWithType:' instead." userInfo:nil];
    return [self initWithType:JKIM_YYImageTypeUnknown];
}

- (instancetype)initWithType:(JKIM_YYImageType)type {
    if (type == JKIM_YYImageTypeUnknown || type >= JKIM_YYImageTypeOther) {
...
#if !JKIM_YYImage_WEBP_ENABLED
    if (type == JKIM_YYImageTypeWebP) {
        NSLog(@"[%s: %d] WebP is not available, check the documentation to see how to install WebP component: https://github.com/ibireme/JKIM_YYImage#installation", __FUNCTION__, __LINE__);
        return nil;
    }
#endif
    ...
    switch (type) {
        case JKIM_YYImageTypeJPEG:
        case JKIM_YYImageTypeJPEG2000: {
            _quality = 0.9;
        } break;
        case JKIM_YYImageTypeTIFF:
        case JKIM_YYImageTypeBMP:
        case JKIM_YYImageTypeGIF:
        case JKIM_YYImageTypeICO:
        case JKIM_YYImageTypeICNS:
        case JKIM_YYImageTypePNG: {
            _quality = 1;
            _lossless = YES;
        } break;
...
- (CGImageDestinationRef)_newImageDestination:(id)dest imageCount:(NSUInteger)count CF_RETURNS_RETAINED {
    ...
    if ([dest isKindOfClass:[NSString class]]) {
        ...
        if (url) {
            destination = CGImageDestinationCreateWithURL((CFURLRef)url, JKIM_YYImageTypeToUTType(_type), count, NULL);
        }
    } else if ([dest isKindOfClass:[NSMutableData class]]) {
        destination = CGImageDestinationCreateWithData((CFMutableDataRef)dest, JKIM_YYImageTypeToUTType(_type), count, NULL);
    }
    return destination;
}
...
- (void)_encodeImageWithDestination:(CGImageDestinationRef)destination imageCount:(NSUInteger)count {
    ...
    for (int i = 0; i < count; i++) {
        @autoreleasepool {
            ...
            if (_type == JKIM_YYImageTypeGIF && count > 1) {
            ...
            if ([imageSrc isKindOfClass:[UIImage class]]) {
                ...
                if (image.imageOrientation != UIImageOrientationUp && image.CGImage) {
                    ...
                    CGImageRef rotated = JKIM_YYCGImageCreateCopyWithOrientation(image.CGImage, image.imageOrientation, info);
                    if (rotated) {
...
- (CGImageRef)_newCGImageFromIndex:(NSUInteger)index decoded:(BOOL)decoded CF_RETURNS_RETAINED {
    ...
    if (image.imageOrientation != UIImageOrientationUp) {
        return JKIM_YYCGImageCreateCopyWithOrientation(imageRef, image.imageOrientation, kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    if (decoded) {
        return JKIM_YYCGImageCreateDecodedCopy(imageRef, YES);
    }
...
- (NSData *)_encodeWithImageIO {
    ...
    NSUInteger count = _type == JKIM_YYImageTypeGIF ? _images.count : 1;
...
- (NSData *)_encodeAPNG {
    ...
    for (int i = 0; i < _images.count; i++) {
        ...
        CFDataRef frameData = JKIM_YYCGImageCreateEncodedData(decoded, JKIM_YYImageTypePNG, 1);
        CFRelease(decoded);
    ...
    if (firstFrameSize.width < canvasWidth || firstFrameSize.height < canvasHeight) {
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, canvasWidth, canvasHeight, 8,
                                                     0, JKIM_YYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
        if (!context) {
            CFRelease(decoded);
            return nil;
        }
        ...
        CFDataRef frameData = JKIM_YYCGImageCreateEncodedData(extendedImage, JKIM_YYImageTypePNG, 1);
        if (!frameData) {
...
- (NSData *)_encodeWebP {
    ...
    for (NSUInteger i = 0; i < _images.count; i++) {
        ...
        if (!image) return nil;
        CFDataRef frameData = JKIM_YYCGImageCreateEncodedWebPData(image, _lossless, _quality, 4, JKIM_YYImagePresetDefault);
        CFRelease(image);
        if (!frameData) return nil;
...
- (NSData *)encode {
    ...
    if (_type == JKIM_YYImageTypePNG) return [self _encodeAPNG];
    if (_type == JKIM_YYImageTypeWebP) return [self _encodeWebP];
    return nil;
}
...
+ (NSData *)encodeImage:(UIImage *)image type:(JKIM_YYImageType)type quality:(CGFloat)quality {
    JKIM_YYImageEncoder *encoder = [[JKIM_YYImageEncoder alloc] initWithType:type];
    encoder.quality = quality;
    [encoder addImage:image duration:0];
    return [encoder encode];
}

+ (NSData *)encodeImageWithDecoder:(JKIM_YYImageDecoder *)decoder type:(JKIM_YYImageType)type quality:(CGFloat)quality {
...
@implementation UIImage (JKIM_YYImageCoder)

- (instancetype)yy_imageByDecoded {
    if (self.yy_isDecodedForDisplay) return self;
    CGImageRef imageRef = self.CGImage;
    if (!imageRef) return self;
    CGImageRef newImageRef = JKIM_YYCGImageCreateDecodedCopy(imageRef, YES);
...
- (BOOL)yy_isDecodedForDisplay {
    if (self.images.count > 1 || [self isKindOfClass:[JKIM_YYSpriteSheetImage class]]) return YES;
...
/// @param forSystem YES: used for system album (PNG/JPEG/GIF), NO: used for JKIM_YYImage (PNG/JPEG/GIF/WebP)
- (NSData *)_yy_dataRepresentationForSystem:(BOOL)forSystem {
    NSData *data = nil;
    if ([self isKindOfClass:[JKIM_YYImage class]]) {
        JKIM_YYImage *image = (id)self;
        if (image.animatedImageData) {
            if (forSystem) { // system only support GIF and PNG
                if (image.animatedImageType == JKIM_YYImageTypeGIF ||
                    image.animatedImageType == JKIM_YYImageTypePNG) {
    ...
    if (!data) {
        ...
        if (imageRef) {
            ...
            if (self.imageOrientation != UIImageOrientationUp) {
                CGImageRef rotated = JKIM_YYCGImageCreateCopyWithOrientation(imageRef, self.imageOrientation, bitmapInfo | alphaInfo);
                if (rotated) {
                    CFRelease(imageRef);
                    imageRef = rotated;
                }
