 ...
 */
@interface _JKIM_YYImageWeakProxy : NSProxy
@property (nonatomic, weak, readonly) id target;
- (instancetype)initWithTarget:(id)target;
+ (instancetype)proxyWithTarget:(id)target;
@end

@implementation _JKIM_YYImageWeakProxy
- (instancetype)initWithTarget:(id)target {
    _target = target;
    return self;
}
+ (instancetype)proxyWithTarget:(id)target {
    return [[_JKIM_YYImageWeakProxy alloc] initWithTarget:target];
}
...
typedef NS_ENUM(NSUInteger, JKIM_YYAnimatedImageType) {
    JKIM_YYAnimatedImageTypeNone = 0,
    JKIM_YYAnimatedImageTypeImage,
    JKIM_YYAnimatedImageTypeHighlightedImage,
    JKIM_YYAnimatedImageTypeImages,
    JKIM_YYAnimatedImageTypeHighlightedImages,
};

@interface JKIM_YYAnimatedImageView() {
    @package
    UIImage <JKIM_YYAnimatedImage> *_curAnimatedImage;
...
@interface _JKIM_YYAnimatedImageViewFetchOperation : NSOperation
@property (nonatomic, weak) JKIM_YYAnimatedImageView *view;
@property (nonatomic, assign) NSUInteger nextIndex;
@property (nonatomic, strong) UIImage <JKIM_YYAnimatedImage> *curImage;
@end

@implementation _JKIM_YYAnimatedImageViewFetchOperation
- (void)main {
    __strong JKIM_YYAnimatedImageView *view = _view;
    if (!view) return;
...
@implementation JKIM_YYAnimatedImageView
...
- (void)resetAnimated {
    if (!_link) {
        ...
        _link = [CADisplayLink displayLinkWithTarget:[_JKIM_YYImageWeakProxy proxyWithTarget:self] selector:@selector(step:)];
        if (_runloopMode) {
...
- (void)setImage:(UIImage *)image {
    ...
    [self setImage:image withType:JKIM_YYAnimatedImageTypeImage];
}

- (void)setHighlightedImage:(UIImage *)highlightedImage {
    if (self.highlightedImage == highlightedImage) return;
    [self setImage:highlightedImage withType:JKIM_YYAnimatedImageTypeHighlightedImage];
}

- (void)setAnimationImages:(NSArray *)animationImages {
    if (self.animationImages == animationImages) return;
    [self setImage:animationImages withType:JKIM_YYAnimatedImageTypeImages];
}
...
- (void)setHighlightedAnimationImages:(NSArray *)highlightedAnimationImages {
    ...
    [self setImage:highlightedAnimationImages withType:JKIM_YYAnimatedImageTypeHighlightedImages];
}
...
- (id)imageForType:(JKIM_YYAnimatedImageType)type {
    switch (type) {
        case JKIM_YYAnimatedImageTypeNone: return nil;
        case JKIM_YYAnimatedImageTypeImage: return self.image;
        case JKIM_YYAnimatedImageTypeHighlightedImage: return self.highlightedImage;
        case JKIM_YYAnimatedImageTypeImages: return self.animationImages;
        case JKIM_YYAnimatedImageTypeHighlightedImages: return self.highlightedAnimationImages;
    }
    return nil;
}

- (JKIM_YYAnimatedImageType)currentImageType {
    JKIM_YYAnimatedImageType curType = JKIM_YYAnimatedImageTypeNone;
    if (self.highlighted) {
        if (self.highlightedAnimationImages.count) curType = JKIM_YYAnimatedImageTypeHighlightedImages;
        else if (self.highlightedImage) curType = JKIM_YYAnimatedImageTypeHighlightedImage;
    }
    if (curType == JKIM_YYAnimatedImageTypeNone) {
        if (self.animationImages.count) curType = JKIM_YYAnimatedImageTypeImages;
        else if (self.image) curType = JKIM_YYAnimatedImageTypeImage;
    }
    return curType;
}

- (void)setImage:(id)image withType:(JKIM_YYAnimatedImageType)type {
    [self stopAnimating];
    if (_link) [self resetAnimated];
    _curFrame = nil;
    switch (type) {
        case JKIM_YYAnimatedImageTypeNone: break;
        case JKIM_YYAnimatedImageTypeImage: super.image = image; break;
        case JKIM_YYAnimatedImageTypeHighlightedImage: super.highlightedImage = image; break;
        case JKIM_YYAnimatedImageTypeImages: super.animationImages = image; break;
        case JKIM_YYAnimatedImageTypeHighlightedImages: super.highlightedAnimationImages = image; break;
    }
    [self imageChanged];
}

- (void)imageChanged {
    JKIM_YYAnimatedImageType newType = [self currentImageType];
    id newVisibleImage = [self imageForType:newType];
    NSUInteger newImageFrameCount = 0;
    BOOL hasContentsRect = NO;
    if ([newVisibleImage isKindOfClass:[UIImage class]] &&
        [newVisibleImage conformsToProtocol:@protocol(JKIM_YYAnimatedImage)]) {
        newImageFrameCount = ((UIImage<JKIM_YYAnimatedImage> *) newVisibleImage).animatedImageFrameCount;
        if (newImageFrameCount > 1) {
            hasContentsRect = [((UIImage<JKIM_YYAnimatedImage> *) newVisibleImage) respondsToSelector:@selector(animatedImageContentsRectAtIndex:)];
        }
    }
    ...
    if (hasContentsRect) {
        CGRect rect = [((UIImage<JKIM_YYAnimatedImage> *) newVisibleImage) animatedImageContentsRectAtIndex:0];
...
- (void)startAnimating {
    JKIM_YYAnimatedImageType type = [self currentImageType];
    if (type == JKIM_YYAnimatedImageTypeImages || type == JKIM_YYAnimatedImageTypeHighlightedImages) {
...
- (void)step:(CADisplayLink *)link {
    UIImage <JKIM_YYAnimatedImage> *image = _curAnimatedImage;
    ...
    if (!bufferIsFull && _requestQueue.operationCount == 0) { // if some work not finished, wait for next opportunity
        _JKIM_YYAnimatedImageViewFetchOperation *operation = [_JKIM_YYAnimatedImageViewFetchOperation new];
        operation.view = self;
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    BOOL ani, multi;
    ani = [self.image conformsToProtocol:@protocol(JKIM_YYAnimatedImage)];
    multi = (ani && ((UIImage <JKIM_YYAnimatedImage> *)self.image).animatedImageFrameCount > 1);
    if (multi) [aCoder encodeObject:self.image forKey:@"YYAnimatedImage"];
    
    ani = [self.highlightedImage conformsToProtocol:@protocol(JKIM_YYAnimatedImage)];
    multi = (ani && ((UIImage <JKIM_YYAnimatedImage> *)self.highlightedImage).animatedImageFrameCount > 1);
