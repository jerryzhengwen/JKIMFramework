...
/**
 ...
 Sample Code:
     ...
     // ani@3x.gif
     JKIM_YYImage *image = [JKIM_YYImage imageNamed:@"ani"];
     JKIM_YYAnimatedImageView *imageView = [JKIM_YYAnimatedImageView alloc] initWithImage:image];
     [view addSubView:imageView];
 */
@interface JKIM_YYAnimatedImageView : UIImageView
...
/**
 ...
 display with JKIM_YYAnimatedImageView.
 
 Subclass a UIImage and implement this protocol, so that instances of that class 
 can be set to JKIM_YYAnimatedImageView.image or JKIM_YYAnimatedImageView.highlightedImage
 to display animation.
 
 See `JKIM_YYImage` and `JKIM_YYFrameImage` for example.
 */
@protocol JKIM_YYAnimatedImage <NSObject>
@required
