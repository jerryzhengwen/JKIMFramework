...
@class JK_YYMemoryCache, JK_YYDiskCache;

NS_ASSUME_NONNULL_BEGIN

/// Image cache type
typedef NS_OPTIONS(NSUInteger, JK_YYImageCacheType) {
    /// No value.
    JK_YYImageCacheTypeNone   = 0,
    
    /// Get/store image with memory cache.
    JK_YYImageCacheTypeMemory = 1 << 0,
    
    /// Get/store image with disk cache.
    JK_YYImageCacheTypeDisk   = 1 << 1,
    
    /// Get/store image with both memory cache and disk cache.
    JK_YYImageCacheTypeAll    = JK_YYImageCacheTypeMemory | JK_YYImageCacheTypeDisk,
};
 ...
 */
@interface JK_YYImageCache : NSObject
...
@property (strong, readonly) JK_YYMemoryCache *memoryCache;

/** The underlying disk cache. see `YYDiskCache` for more information.*/
@property (strong, readonly) JK_YYDiskCache *diskCache;
...
- (void)setImage:(nullable UIImage *)image
       imageData:(nullable NSData *)imageData
          forKey:(NSString *)key
        withType:(JK_YYImageCacheType)type;
...
/**
 ...
 @return The image associated with key, or nil if no image is associated with key.
 */
- (nullable UIImage *)getImageForKey:(NSString *)key withType:(JK_YYImageCacheType)type;
...
- (void)getImageForKey:(NSString *)key
              withType:(JK_YYImageCacheType)type
             withBlock:(void(^)(UIImage * _Nullable image, JK_YYImageCacheType type))block;
