...
static BOOL YYCGImageLastPixelFilled(CGImageRef image) {
    ...
    CGContextRef ctx = CGBitmapContextCreate(NULL, 1, 1, 8, 0, JK_YYCGColorSpaceGetDeviceRGB(), kCGImageAlphaPremultipliedFirst | kCGBitmapByteOrderDefault);
    if (!ctx) return NO;
...
@interface JK_YYWebImageWeakProxy : NSProxy
@property (nonatomic, weak, readonly) id target;
- (instancetype)initWithTarget:(id)target;
+ (instancetype)proxyWithTarget:(id)target;
@end

@implementation JK_YYWebImageWeakProxy
- (instancetype)initWithTarget:(id)target {
    _target = target;
    return self;
}
+ (instancetype)proxyWithTarget:(id)target {
    return [[JK_YYWebImageWeakProxy alloc] initWithTarget:target];
}
...
@interface JK_YYWebImageOperation() <NSURLConnectionDelegate>
...
@property (nonatomic, strong) JK_YYImageDecoder *progressiveDecoder;
...
@property (nonatomic, copy) JK_YYWebImageProgressBlock progress;
@property (nonatomic, copy) JK_YYWebImageTransformBlock transform;
@property (nonatomic, copy) JK_YYWebImageCompletionBlock completion;
@end


@implementation JK_YYWebImageOperation
...
- (void)dealloc {
    ...
    if ([self isExecuting]) {
        ...
        if (_connection) {
            [_connection cancel];
            if (![_request.URL isFileURL] && (_options & JK_YYWebImageOptionShowNetworkActivity)) {
                [JK_YYWebImageManager decrementNetworkActivityCount];
            }
        }
        if (_completion) {
            @autoreleasepool {
                _completion(nil, _request.URL, JK_YYWebImageFromNone, JK_YYWebImageStageCancelled, nil);
            }
        }
    }
    [_lock unlock];
...
- (void)_startOperation {
    ...
    @autoreleasepool {
        ...
        if (_cache &&
            !(_options & JK_YYWebImageOptionUseNSURLCache) &&
            !(_options & JK_YYWebImageOptionRefreshImageCache)) {
            UIImage *image = [_cache getImageForKey:_cacheKey withType:JK_YYImageCacheTypeMemory];
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    if (_completion) _completion(image, _request.URL, JK_YYWebImageFromMemoryCache, JK_YYWebImageStageFinished, nil);
                }
                [self _finish];
                [_lock unlock];
                return;
            }
            if (!(_options & JK_YYWebImageOptionIgnoreDiskCache)) {
                __weak typeof(self) _self = self;
                dispatch_async([self.class _imageQueue], ^{
                    __strong typeof(_self) self = _self;
                    if (!self || [self isCancelled]) return;
                    UIImage *image = [self.cache getImageForKey:self.cacheKey withType:JK_YYImageCacheTypeDisk];
                    if (image) {
                        [self.cache setImage:image imageData:nil forKey:self.cacheKey withType:JK_YYImageCacheTypeMemory];
...
- (void)_startRequest:(id)object {
    if ([self isCancelled]) return;
    @autoreleasepool {
        if ((_options & JK_YYWebImageOptionIgnoreFailedURL) && URLBlackListContains(_request.URL)) {
            ...
            [_lock lock];
            if (![self isCancelled]) {
                if (_completion) _completion(nil, _request.URL, JK_YYWebImageFromNone, JK_YYWebImageStageFinished, error);
            }
            [self _finish];
            [_lock unlock];
            return;
        ...
        [_lock lock];
        if (![self isCancelled]) {
            _connection = [[NSURLConnection alloc] initWithRequest:_request delegate:[JK_YYWebImageWeakProxy proxyWithTarget:self]];
            if (![_request.URL isFileURL] && (_options & JK_YYWebImageOptionShowNetworkActivity)) {
                [JK_YYWebImageManager incrementNetworkActivityCount];
            }
        }
        [_lock unlock];
    }
...
- (void)_cancelOperation {
    @autoreleasepool {
        ...
        [_connection cancel];
        _connection = nil;
        if (_completion) _completion(nil, _request.URL, JK_YYWebImageFromNone, JK_YYWebImageStageCancelled, nil);
        [self _endBackgroundTask];
    }
}
...
- (void)_didReceiveImageFromDiskCache:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (image) {
                if (_completion) _completion(image, _request.URL, JK_YYWebImageFromDiskCache, JK_YYWebImageStageFinished, nil);
                [self _finish];
            } else {
...
- (void)_didReceiveImageFromWeb:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_cache) {
                if (image || (_options & JK_YYWebImageOptionRefreshImageCache)) {
                    NSData *data = _data;
                    dispatch_async([JK_YYWebImageOperation _imageQueue], ^{
                        [_cache setImage:image imageData:data forKey:_cacheKey withType:JK_YYImageCacheTypeAll];
                    });
                }
            }
            _data = nil;
            ...
            if (!image) {
                ...
                if (_options & JK_YYWebImageOptionIgnoreFailedURL) {
            ...
            if (_completion) _completion(image, _request.URL, JK_YYWebImageFromRemote, JK_YYWebImageStageFinished, error);
            [self _finish];
        }
        [_lock unlock];
    }
...
- (void)connection:(NSURLConnection *)connection willSendRequestForAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
    @autoreleasepool {
        if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
            if (!(_options & JK_YYWebImageOptionAllowInvalidSSLCertificates) &&
...
- (NSCachedURLResponse *)connection:(NSURLConnection *)connection willCacheResponse:(NSCachedURLResponse *)cachedResponse {
    ...
    if (_options & JK_YYWebImageOptionUseNSURLCache) {
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    @autoreleasepool {
        ...
        /*--------------------------- progressive ----------------------------*/
        BOOL progressive = (_options & JK_YYWebImageOptionProgressive) > 0;
        BOOL progressiveBlur = (_options & JK_YYWebImageOptionProgressiveBlur) > 0;
        ...
        if (!_progressiveDecoder) {
            _progressiveDecoder = [[JK_YYImageDecoder alloc] initWithScale:[UIScreen mainScreen].scale];
        }
        [_progressiveDecoder updateData:_data final:NO];
        if ([self isCancelled]) return;
        
        if (_progressiveDecoder.type == JK_YYImageTypeUnknown ||
            _progressiveDecoder.type == JK_YYImageTypeWebP ||
            _progressiveDecoder.type == JK_YYImageTypeOther) {
            _progressiveDecoder = nil;
            _progressiveIgnored = YES;
            return;
        }
        if (progressiveBlur) { // only support progressive JPEG and interlaced PNG
            if (_progressiveDecoder.type != JK_YYImageTypeJPEG &&
                _progressiveDecoder.type != JK_YYImageTypePNG) {
                _progressiveDecoder = nil;
                _progressiveIgnored = YES;
                return;
            }
        ...
        if (!progressiveBlur) {
            JK_YYImageFrame *frame = [_progressiveDecoder frameAtIndex:0 decodeForDisplay:YES];
            if (frame.image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(frame.image, _request.URL, JK_YYWebImageFromRemote, JK_YYWebImageStageProgress, nil);
            ...
            } else if (_progressiveDecoder.type == JK_YYImageTypePNG) {
                if (!_progressiveDetected) {
                    NSDictionary *dic = [_progressiveDecoder framePropertiesAtIndex:0];
            ...
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(image, _request.URL, JK_YYWebImageFromRemote, JK_YYWebImageStageProgress, nil);
                    _lastProgressiveDecodeTimestamp = now;
                }
                [_lock unlock];
            }
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            dispatch_async([self.class _imageQueue], ^{
                ...
                BOOL shouldDecode = (self.options & JK_YYWebImageOptionIgnoreImageDecoding) == 0;
                BOOL allowAnimation = (self.options & JK_YYWebImageOptionIgnoreAnimatedImage) == 0;
                UIImage *image;
                BOOL hasAnimation = NO;
                if (allowAnimation) {
                    image = [[JK_YYImage alloc] initWithData:self.data scale:[UIScreen mainScreen].scale];
                    if (shouldDecode) image = [image yy_imageByDecoded];
                    if ([((JK_YYImage *)image) animatedImageFrameCount] > 1) {
                        hasAnimation = YES;
                    }
                } else {
                    JK_YYImageDecoder *decoder = [JK_YYImageDecoder decoderWithData:self.data scale:[UIScreen mainScreen].scale];
                 ...
                 */
                JK_YYImageType imageType = JK_YYImageDetectType((__bridge CFDataRef)self.data);
                switch (imageType) {
                    case JK_YYImageTypeJPEG:
                    case JK_YYImageTypeGIF:
                    case JK_YYImageTypePNG:
                    case JK_YYImageTypeWebP: { // save to disk cache
                        if (!hasAnimation) {
                            if (imageType == JK_YYImageTypeGIF ||
                                imageType == JK_YYImageTypeWebP) {
            ...
            if (![self.request.URL isFileURL] && (self.options & JK_YYWebImageOptionShowNetworkActivity)) {
                [JK_YYWebImageManager decrementNetworkActivityCount];
            }
        }
        [_lock unlock];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_completion) {
                _completion(nil, _request.URL, JK_YYWebImageFromNone, JK_YYWebImageStageFinished, error);
            }
            _connection = nil;
            _data = nil;
            if (![_request.URL isFileURL] && (_options & JK_YYWebImageOptionShowNetworkActivity)) {
    ...
    @autoreleasepool {
        ...
        if ([self isCancelled]) {
            ...
            if (!_request) {
                ...
                if ((_options & JK_YYWebImageOptionAllowBackgroundTask) && _YYSharedApplication()) {
