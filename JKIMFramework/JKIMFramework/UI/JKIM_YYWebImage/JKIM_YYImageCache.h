...
@class JKIM_YYMemoryCache, JKIM_YYDiskCache;

NS_ASSUME_NONNULL_BEGIN

/// Image cache type
typedef NS_OPTIONS(NSUInteger, JKIM_YYImageCacheType) {
    /// No value.
    JKIM_YYImageCacheTypeNone   = 0,
    
    /// Get/store image with memory cache.
    JKIM_YYImageCacheTypeMemory = 1 << 0,
    
    /// Get/store image with disk cache.
    JKIM_YYImageCacheTypeDisk   = 1 << 1,
    
    /// Get/store image with both memory cache and disk cache.
    JKIM_YYImageCacheTypeAll    = JKIM_YYImageCacheTypeMemory | JKIM_YYImageCacheTypeDisk,
};


/**
 JKIM_YYImageCache is a cache that stores UIImage and image data based on memory cache and disk cache.
 ...
 */
@interface JKIM_YYImageCache : NSObject
...
/** The underlying memory cache. see `JKIM_YYMemoryCache` for more information.*/
@property (strong, readonly) JKIM_YYMemoryCache *memoryCache;

/** The underlying disk cache. see `JKIM_YYDiskCache` for more information.*/
@property (strong, readonly) JKIM_YYDiskCache *diskCache;

/**
 Whether decode animated image when fetch image from disk cache. Default is YES.
 
 @discussion When fetch image from disk cache, it will use 'JKIM_YYImage' to decode 
...
/**
 ...
 @return  The singleton JKIM_YYImageCache instance.
 */
...
/**
 ...
 @discussion If the `type` contain `JKIM_YYImageCacheTypeMemory`, then the `image` will 
 be stored in the memory cache; `imageData` will be used instead if `image` is nil.
 If the `type` contain `JKIM_YYImageCacheTypeDisk`, then the `imageData` will
...
- (void)setImage:(nullable UIImage *)image
       imageData:(nullable NSData *)imageData
          forKey:(NSString *)key
        withType:(JKIM_YYImageCacheType)type;
...
/**
 Returns a Boolean value that indicates whether a given key is in cache.
 If the image is not in memory and the `type` contains `JKIM_YYImageCacheTypeDisk`,
...
/**
 ...
 @return The image associated with key, or nil if no image is associated with key.
 */
- (nullable UIImage *)getImageForKey:(NSString *)key withType:(JKIM_YYImageCacheType)type;
...
- (void)getImageForKey:(NSString *)key
              withType:(JKIM_YYImageCacheType)type
             withBlock:(void(^)(UIImage * _Nullable image, JKIM_YYImageCacheType type))block;
