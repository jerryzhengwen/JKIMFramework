//
//  JKIM_YYKVStorage.h
//  JKIM_YYCache <https://github.com/ibireme/JKIM_YYCache>
//
...
/**
 JKIM_YYKVStorageItem is used by `JKIM_YYKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface JKIM_YYKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `JKIM_YYKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache), 
   use JKIM_YYKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
   use JKIM_YYKVStorageTypeFile to get better performance.
 * You can use JKIM_YYKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, JKIM_YYKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    JKIM_YYKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    JKIM_YYKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    JKIM_YYKVStorageTypeMixed = 2,
};



/**
 JKIM_YYKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for JKIM_YYKVStorage is `initWithPath:type:`. 
 ...
 */
@interface JKIM_YYKVStorage : NSObject
...
@property (nonatomic, readonly) JKIM_YYKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(JKIM_YYKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is JKIM_YYKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is JKIM_YYKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is JKIM_YYKVStorageTypeMixed, then the item.value will be saved to file 
 system if the item.filename is not empty, otherwise it will be saved to sqlite.
 
 @param item  An item.
 @return Whether succeed.
 */
- (BOOL)saveItem:(JKIM_YYKVStorageItem *)item;

/**
 Save an item or update the item with 'key' if it already exists.
 
 @discussion This method will save the key-value pair to sqlite. If the `type` is
 JKIM_YYKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is JKIM_YYKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is JKIM_YYKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is JKIM_YYKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable JKIM_YYKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable JKIM_YYKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `JKIM_YYKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<JKIM_YYKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `JKIM_YYKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<JKIM_YYKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
