...
static JK_YYDiskCache *_YYDiskCacheGetGlobal(NSString *path) {
...
static void _YYDiskCacheSetGlobal(JK_YYDiskCache *cache) {
...
- (instancetype)initWithPath:(NSString *)path
    ...
    JK_YYDiskCache *globalCache = _YYDiskCacheGetGlobal(path);
    if (globalCache) return globalCache;
    
    JK_YYKVStorageType type;
    if (threshold == 0) {
        type = JK_YYKVStorageTypeFile;
    } else if (threshold == NSUIntegerMax) {
        type = JK_YYKVStorageTypeSQLite;
    } else {
        type = JK_YYKVStorageTypeMixed;
    }
    
    JK_YYKVStorage *kv = [[JK_YYKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    JK_YYKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [JK_YYDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [JK_YYDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (!value) return;
    NSString *filename = nil;
    if (_kv.type != JK_YYKVStorageTypeSQLite) {
...
- (NSString *)filePathForkey:(NSString *)key
{
    if (!key) return nil;
    Lock();
    JK_YYKVStorageItem *item = [_kv getItemInfoForKey:key];
    Unlock();
