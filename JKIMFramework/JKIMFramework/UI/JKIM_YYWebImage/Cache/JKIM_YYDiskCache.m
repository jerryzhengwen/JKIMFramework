...
static void _JKIM_YYDiskCacheInitGlobal() {
...
static JKIM_YYDiskCache *_JKIM_YYDiskCacheGetGlobal(NSString *path) {
    if (path.length == 0) return nil;
    _JKIM_YYDiskCacheInitGlobal();
...
static void _JKIM_YYDiskCacheSetGlobal(JKIM_YYDiskCache *cache) {
...
@implementation JKIM_YYDiskCache {
    JKIM_YYKVStorage *_kv;
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"JKIM_YYDiskCache init error" reason:@"JKIM_YYDiskCache must be initialized with a path. Use 'initWithPath:' or 'initWithPath:inlineThreshold:' instead." userInfo:nil];
...
- (instancetype)initWithPath:(NSString *)path
    ...
    JKIM_YYDiskCache *globalCache = _JKIM_YYDiskCacheGetGlobal(path);
    if (globalCache) return globalCache;
    
    JKIM_YYKVStorageType type;
    if (threshold == 0) {
        type = JKIM_YYKVStorageTypeFile;
    } else if (threshold == NSUIntegerMax) {
        type = JKIM_YYKVStorageTypeSQLite;
    } else {
        type = JKIM_YYKVStorageTypeMixed;
    }
    
    JKIM_YYKVStorage *kv = [[JKIM_YYKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
    ...
    [self _trimRecursively];
    _JKIM_YYDiskCacheSetGlobal(self);
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    JKIM_YYKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [JKIM_YYDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [JKIM_YYDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (!value) return;
    NSString *filename = nil;
    if (_kv.type != JKIM_YYKVStorageTypeSQLite) {
