...
/**
 JKIM_YYMemoryCache is a fast in-memory cache that stores key-value pairs.
 ...
 JKIM_YYMemoryCache objects differ from NSCache in a few ways:
 ...
 * It can be configured to automatically evict objects when receive memory 
 ...
 The time of `Access Methods` in JKIM_YYMemoryCache is typically in constant time (O(1)).
 */
@interface JKIM_YYMemoryCache : NSObject
 ...
 */
@property (nullable, copy) void(^didReceiveMemoryWarningBlock)(JKIM_YYMemoryCache *cache);

/**
 A block to be executed when the app enter background.
 The default value is nil.
 */
@property (nullable, copy) void(^didEnterBackgroundBlock)(JKIM_YYMemoryCache *cache);
