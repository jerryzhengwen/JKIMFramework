...
static BOOL YYCGImageLastPixelFilled(CGImageRef image) {
    ...
    CGContextRef ctx = CGBitmapContextCreate(NULL, 1, 1, 8, 0, JKIM_YYCGColorSpaceGetDeviceRGB(), kCGImageAlphaPremultipliedFirst | kCGBitmapByteOrderDefault);
    if (!ctx) return NO;
...
@interface _JKIM_YYWebImageWeakProxy : NSProxy
@property (nonatomic, weak, readonly) id target;
- (instancetype)initWithTarget:(id)target;
+ (instancetype)proxyWithTarget:(id)target;
@end

@implementation _JKIM_YYWebImageWeakProxy
- (instancetype)initWithTarget:(id)target {
    _target = target;
    return self;
}
+ (instancetype)proxyWithTarget:(id)target {
    return [[_JKIM_YYWebImageWeakProxy alloc] initWithTarget:target];
}
...
@interface JKIM_YYWebImageOperation() <NSURLConnectionDelegate>
...
@property (nonatomic, strong) JKIM_YYImageDecoder *progressiveDecoder;
...
@property (nonatomic, copy) JKIM_YYWebImageProgressBlock progress;
@property (nonatomic, copy) JKIM_YYWebImageTransformBlock transform;
@property (nonatomic, copy) JKIM_YYWebImageCompletionBlock completion;
@end


@implementation JKIM_YYWebImageOperation
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"JKIM_YYWebImageOperation init error" reason:@"JKIM_YYWebImageOperation must be initialized with a request. Use the designated initializer to init." userInfo:nil];
...
- (void)dealloc {
    ...
    if ([self isExecuting]) {
        ...
        if (_connection) {
            [_connection cancel];
            if (![_request.URL isFileURL] && (_options & JKIM_YYWebImageOptionShowNetworkActivity)) {
                [JKIM_YYWebImageManager decrementNetworkActivityCount];
            }
        }
        if (_completion) {
            @autoreleasepool {
                _completion(nil, _request.URL, JKIM_YYWebImageFromNone, JKIM_YYWebImageStageCancelled, nil);
            }
        }
    }
    [_lock unlock];
...
- (void)_startOperation {
    ...
    @autoreleasepool {
        ...
        if (_cache &&
            !(_options & JKIM_YYWebImageOptionUseNSURLCache) &&
            !(_options & JKIM_YYWebImageOptionRefreshImageCache)) {
            UIImage *image = [_cache getImageForKey:_cacheKey withType:JKIM_YYImageCacheTypeMemory];
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    if (_completion) _completion(image, _request.URL, JKIM_YYWebImageFromMemoryCache, JKIM_YYWebImageStageFinished, nil);
                }
                [self _finish];
                [_lock unlock];
                return;
            }
            if (!(_options & JKIM_YYWebImageOptionIgnoreDiskCache)) {
                __weak typeof(self) _self = self;
                dispatch_async([self.class _imageQueue], ^{
                    __strong typeof(_self) self = _self;
                    if (!self || [self isCancelled]) return;
                    UIImage *image = [self.cache getImageForKey:self.cacheKey withType:JKIM_YYImageCacheTypeDisk];
                    if (image) {
                        [self.cache setImage:image imageData:nil forKey:self.cacheKey withType:JKIM_YYImageCacheTypeMemory];
...
- (void)_startRequest:(id)object {
    if ([self isCancelled]) return;
    @autoreleasepool {
        if ((_options & JKIM_YYWebImageOptionIgnoreFailedURL) && URLBlackListContains(_request.URL)) {
            ...
            [_lock lock];
            if (![self isCancelled]) {
                if (_completion) _completion(nil, _request.URL, JKIM_YYWebImageFromNone, JKIM_YYWebImageStageFinished, error);
            }
            [self _finish];
            [_lock unlock];
            return;
        ...
        [_lock lock];
        if (![self isCancelled]) {
            _connection = [[NSURLConnection alloc] initWithRequest:_request delegate:[_JKIM_YYWebImageWeakProxy proxyWithTarget:self]];
            if (![_request.URL isFileURL] && (_options & JKIM_YYWebImageOptionShowNetworkActivity)) {
                [JKIM_YYWebImageManager incrementNetworkActivityCount];
            }
        }
        [_lock unlock];
    }
...
- (void)_cancelOperation {
    @autoreleasepool {
        ...
        [_connection cancel];
        _connection = nil;
        if (_completion) _completion(nil, _request.URL, JKIM_YYWebImageFromNone, JKIM_YYWebImageStageCancelled, nil);
        [self _endBackgroundTask];
    }
}
...
- (void)_didReceiveImageFromDiskCache:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (image) {
                if (_completion) _completion(image, _request.URL, JKIM_YYWebImageFromDiskCache, JKIM_YYWebImageStageFinished, nil);
                [self _finish];
            } else {
...
- (void)_didReceiveImageFromWeb:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_cache) {
                if (image || (_options & JKIM_YYWebImageOptionRefreshImageCache)) {
                    NSData *data = _data;
                    dispatch_async([JKIM_YYWebImageOperation _imageQueue], ^{
                        JKIM_YYImageCacheType cacheType = (_options & JKIM_YYWebImageOptionIgnoreDiskCache) ? JKIM_YYImageCacheTypeMemory : JKIM_YYImageCacheTypeAll;
            ...
            if (!image) {
                ...
                if (_options & JKIM_YYWebImageOptionIgnoreFailedURL) {
            ...
            if (_completion) _completion(image, _request.URL, JKIM_YYWebImageFromRemote, JKIM_YYWebImageStageFinished, error);
            [self _finish];
        }
        [_lock unlock];
    }
...
- (void)connection:(NSURLConnection *)connection willSendRequestForAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
    @autoreleasepool {
        if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
            if (!(_options & JKIM_YYWebImageOptionAllowInvalidSSLCertificates) &&
...
- (NSCachedURLResponse *)connection:(NSURLConnection *)connection willCacheResponse:(NSCachedURLResponse *)cachedResponse {
    ...
    if (_options & JKIM_YYWebImageOptionUseNSURLCache) {
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    @autoreleasepool {
        ...
        /*--------------------------- progressive ----------------------------*/
        BOOL progressive = (_options & JKIM_YYWebImageOptionProgressive) > 0;
        BOOL progressiveBlur = (_options & JKIM_YYWebImageOptionProgressiveBlur) > 0;
        ...
        if (!_progressiveDecoder) {
            _progressiveDecoder = [[JKIM_YYImageDecoder alloc] initWithScale:[UIScreen mainScreen].scale];
        }
        [_progressiveDecoder updateData:_data final:NO];
        if ([self isCancelled]) return;
        
        if (_progressiveDecoder.type == JKIM_YYImageTypeUnknown ||
            _progressiveDecoder.type == JKIM_YYImageTypeWebP ||
            _progressiveDecoder.type == JKIM_YYImageTypeOther) {
            _progressiveDecoder = nil;
            _progressiveIgnored = YES;
            return;
        }
        if (progressiveBlur) { // only support progressive JPEG and interlaced PNG
            if (_progressiveDecoder.type != JKIM_YYImageTypeJPEG &&
                _progressiveDecoder.type != JKIM_YYImageTypePNG) {
                _progressiveDecoder = nil;
                _progressiveIgnored = YES;
                return;
            }
        ...
        if (!progressiveBlur) {
            JKIM_YYImageFrame *frame = [_progressiveDecoder frameAtIndex:0 decodeForDisplay:YES];
            if (frame.image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(frame.image, _request.URL, JKIM_YYWebImageFromRemote, JKIM_YYWebImageStageProgress, nil);
            ...
            } else if (_progressiveDecoder.type == JKIM_YYImageTypePNG) {
                if (!_progressiveDetected) {
                    NSDictionary *dic = [_progressiveDecoder framePropertiesAtIndex:0];
            ...
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(image, _request.URL, JKIM_YYWebImageFromRemote, JKIM_YYWebImageStageProgress, nil);
                    _lastProgressiveDecodeTimestamp = now;
                }
                [_lock unlock];
            }
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            dispatch_async([self.class _imageQueue], ^{
                ...
                BOOL shouldDecode = (self.options & JKIM_YYWebImageOptionIgnoreImageDecoding) == 0;
                BOOL allowAnimation = (self.options & JKIM_YYWebImageOptionIgnoreAnimatedImage) == 0;
                UIImage *image;
                BOOL hasAnimation = NO;
                if (allowAnimation) {
                    image = [[JKIM_YYImage alloc] initWithData:self.data scale:[UIScreen mainScreen].scale];
                    if (shouldDecode) image = [image yy_imageByDecoded];
                    if ([((JKIM_YYImage *)image) animatedImageFrameCount] > 1) {
                        hasAnimation = YES;
                    }
                } else {
                    JKIM_YYImageDecoder *decoder = [JKIM_YYImageDecoder decoderWithData:self.data scale:[UIScreen mainScreen].scale];
                 ...
                 */
                JKIM_YYImageType imageType = JKIM_YYImageDetectType((__bridge CFDataRef)self.data);
                switch (imageType) {
                    case JKIM_YYImageTypeJPEG:
                    case JKIM_YYImageTypeGIF:
                    case JKIM_YYImageTypePNG:
                    case JKIM_YYImageTypeWebP: { // save to disk cache
                        if (!hasAnimation) {
                            if (imageType == JKIM_YYImageTypeGIF ||
                                imageType == JKIM_YYImageTypeWebP) {
            ...
            if (![self.request.URL isFileURL] && (self.options & JKIM_YYWebImageOptionShowNetworkActivity)) {
                [JKIM_YYWebImageManager decrementNetworkActivityCount];
            }
        }
        [_lock unlock];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_completion) {
                _completion(nil, _request.URL, JKIM_YYWebImageFromNone, JKIM_YYWebImageStageFinished, error);
            }
            _connection = nil;
            _data = nil;
            if (![_request.URL isFileURL] && (_options & JKIM_YYWebImageOptionShowNetworkActivity)) {
    ...
    @autoreleasepool {
        ...
        if ([self isCancelled]) {
            ...
            if (!_request) {
                ...
                if ((_options & JKIM_YYWebImageOptionAllowBackgroundTask) && _YYSharedApplication()) {
