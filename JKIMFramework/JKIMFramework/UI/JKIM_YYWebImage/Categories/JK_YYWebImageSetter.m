...
NSString *const JK_YYWebImageFadeAnimationKey = @"YYWebImageFade";
const NSTimeInterval JK_YYWebImageFadeTime = 0.2;
const NSTimeInterval JK_YYWebImageProgressiveFadeTime = 0.4;


@implementation JK_YYWebImageSetter {
...
- (int32_t)setOperationWithSentinel:(int32_t)sentinel
    ...
    if (sentinel != _sentinel) {
        if (completion) completion(nil, imageURL, JK_YYWebImageFromNone, JK_YYWebImageStageCancelled, nil);
        return _sentinel;
    }
    ...
    if (!operation && completion) {
        ...
        completion(nil, imageURL, JK_YYWebImageFromNone, JK_YYWebImageStageFinished, [NSError errorWithDomain:@"com.ibireme.webimage" code:-1 userInfo:userInfo]);
    }
