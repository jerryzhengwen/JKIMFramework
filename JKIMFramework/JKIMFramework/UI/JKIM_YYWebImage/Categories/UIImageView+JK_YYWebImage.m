...
#import "UIImageView+JK_YYWebImage.h"
#import "JK_YYWebImageOperation.h"
#import "JK_YYWebImageSetter.h"
#import <objc/runtime.h>

// Dummy class for category
@interface JK_UIImageView_YYWebImage : NSObject @end
@implementation JK_UIImageView_YYWebImage @end

static int JK_YYWebImageSetterKey;
static int _YYWebImageHighlightedSetterKey;


@implementation UIImageView (JK_YYWebImage)
...
- (void)yy_setImageWithURL:(NSURL *)imageURL
    ...
    _JK_yy_dispatch_sync_on_main_queue(^{
        if ((options & JK_YYWebImageOptionSetImageWithFadeAnimation) &&
            ...
            if (!self.highlighted) {
                [self.layer removeAnimationForKey:JK_YYWebImageFadeAnimationKey];
            }
        }
            ...
            JK_YYWebImageCompletionBlock _completion = ^(UIImage *image, NSURL *url, JK_YYWebImageFromType from, JK_YYWebImageStage stage, NSError *error) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (setImage && self && !sentinelChanged) {
                        BOOL showFade = ((options & JK_YYWebImageOptionSetImageWithFadeAnimation) && !self.highlighted);
                        if (showFade) {
                            ...
                            [self.layer addAnimation:transition forKey:JK_YYWebImageFadeAnimationKey];
                        }
                        self.image = image;
                    }
                    if (completion) {
...
- (NSURL *)yy_highlightedImageURL {
    JK_YYWebImageSetter *setter = objc_getAssociatedObject(self, &_YYWebImageHighlightedSetterKey);
    return setter.imageURL;
}
...
- (void)yy_setHighlightedImageWithURL:(NSURL *)imageURL options:(JK_YYWebImageOptions)options {
    [self yy_setHighlightedImageWithURL:imageURL
                            placeholder:nil
                                options:options
                                manager:nil
