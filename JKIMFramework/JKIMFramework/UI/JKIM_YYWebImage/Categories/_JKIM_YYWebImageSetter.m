...
NSString *const _JKIM_YYWebImageFadeAnimationKey = @"JKIM_YYWebImageFade";
const NSTimeInterval _JKIM_YYWebImageFadeTime = 0.2;
const NSTimeInterval _JKIM_YYWebImageProgressiveFadeTime = 0.4;


@implementation _JKIM_YYWebImageSetter {
...
- (int32_t)setOperationWithSentinel:(int32_t)sentinel
    ...
    if (sentinel != _sentinel) {
        if (completion) completion(nil, imageURL, JKIM_YYWebImageFromNone, JKIM_YYWebImageStageCancelled, nil);
        return _sentinel;
    }
    ...
    if (!operation && completion) {
        NSDictionary *userInfo = @{ NSLocalizedDescriptionKey : @"JKIM_YYWebImageOperation create failed." };
        completion(nil, imageURL, JKIM_YYWebImageFromNone, JKIM_YYWebImageStageFinished, [NSError errorWithDomain:@"com.ibireme.webimage" code:-1 userInfo:userInfo]);
    }
